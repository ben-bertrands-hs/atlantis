...
func TestRenderErr(t *testing.T) {
	...
	r := events.NewMarkdownRenderer(
		...
		false,      // EnableExtendedPlanSummary
...
func TestRenderProjectResultsWithEnableExtendedPlanSummary(t *testing.T) {
...
Plan: 2 to add, 1 to change, 0 to destroy.`
	...
	cases := []struct {
	...
	}{
		{
			...
			EnableSummary:    false,
			ExpectedInOutput: "+ will be created",
			ShouldContain:    false,
		},
		{
			Description:      "Resource summary enabled",
			EnableSummary:    true,
			ExpectedInOutput: "+ will be created (2)",
			ShouldContain:    true,
		},
	}
	...
	for _, c := range cases {
		t.Run(c.Description, func(t *testing.T) {
			r := events.NewMarkdownRenderer(
				...
				false,           // disableRepoLocking
				false,           // enableDiffMarkdownFormat
				c.EnableSummary, // EnableExtendedPlanSummary
				"",              // markdownTemplateOverridesDir
				"atlantis",      // executableName
				false,           // hideUnchangedPlanComments
				false,           // quietPolicyChecks
			...
			if c.ShouldContain {
				...
				if !strings.Contains(output, "! will be updated in-place (1)") {
